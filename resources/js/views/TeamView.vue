...
<template>
  <section class="px-4 py-14 sm:px-6 sm:py-16">
    <div class="mx-auto w-full max-w-6xl">
      <p class="text-xs uppercase tracking-[0.18em] text-dim">Team</p>
      <h1 class="mt-3 text-3xl font-semibold text-ink sm:text-4xl">Solo-led engineering with senior accountability.</h1>
      <p class="mt-4 max-w-3xl text-sm leading-7 text-muted sm:text-base">
        ForEachDev is led directly by Mateus Galasso, with end-to-end ownership from architecture to delivery.
      </p>

      <div class="mt-10 space-y-5">
        <article v-for="member in team" :key="member.name" class="panel-card">
